...
多重継承ができる。クラス定義の際にカッコ内に書いた親の順序でメソッド解決順序が決まる。

### 10.5

#### 10.5.4

ゲッターの前には@propertyというデコレーターをつける

#### 10.5.6

クラス属性はインスタンスに受け継がれる。クラス属性を変更した場合、既存のインスタンスの属性は変わらない。

### 10.6

メソッドの分類。

- デコレーターがついていないメソッドはインスタンスメソッドで、第1引数はself
- @classmethodがついているメソッドはクラスメソッドで、第１引数はcls
- @staticmethodがついているメソッドは静的メソッドで、第１引数はselfでもclsでもない

### 10.8

`__str__()`メソッドを定義すると、print(object)を実行したときに__str__()の戻り値を表示する
...
sys.pathにPythonインタプリタが参照するすべてのフォルダパスが書いてある。ここに書いてある順にモジュールの検索を行う。ふつうはカレントディレクトリをはじめに参照するが、sys.pathを書き換えることもできる
...
encode()関数は、文字列をバイト列に分解する。引数にutf-8などの文字コードを指定する。
